<template>
    <div id="cinema" v-if="list.length>0">
        <div class="citem" v-for="(item, index) in district">
            ...
            <div class="list" style="background: #fff;" v-if='pinyin == item.pinyin'>
                    <div class="shop" v-for="shop in list" v-if="shop.district.pinyin == item.pinyin">
                        <router-link :to="{ name: 'cinemadetail', params: { id: shop.id, title: shop.name }}">
                            ...
                            <div class="cright1">¥{{shop.minimumPrice}}</div>
                        </router-link>
                    </div>
            </div>
        </div>
    ...
    export default {
        ...
        created() {
            this.$watch('district',function() {
                if (this.district) {
                    this.pinyin = this.district[0].pinyin;
                }                
            })
            if (this.list.length == 0) {
                let id = this.$route.params.id
                this.$store.dispatch('getCinemaListById', id)
            }            
        },
        computed: {
            ...mapGetters({
